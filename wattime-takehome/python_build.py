...
class build_pipeline:
    ...
    def make_clean(self):
        ...
        #subprocess.check_call([sys.executable, '-m', 'pip', 'install', 'my_package'])
        result = subprocess.run(['jupyter-book', 'clean', 'jupyter-book'], capture_output=True, text=True)
        print(result.stdout)
        print(result.stderr)
    ...
    def make_html(self):
        ...
        #subprocess.check_call([sys.executable, '-m', 'pip', 'install', 'my_package'])
        result = subprocess.run(['jupyter-book', 'build', 'jupyter-book'], capture_output=True, text=True)
        print(result.stdout)
        print(result.stderr)
        ##print("i make it")
...
class deploy_pipeline:
    ...
    def deploy(self,cname):
        ...
        #subprocess.check_call([sys.executable, '-m', 'pip', 'install', 'my_package'])
        result = subprocess.run(['ghp-import', '-n', '-p', '-f', '-c', cname, 'jupyter-book/_build/html' ], capture_output=True, text=True)
        ##print("Output")
